...
static __global__ void diff_unwrap_phase_kernel1(const emxArray_creal32_T yq,
                                                 const int nx,
                                                 emxArray_real32_T b_y);

static __global__ void diff_unwrap_phase_kernel2(const emxArray_real32_T b_y,
                                                 const int vstride,
                                                 const int i1, const int vlen,
...
// Return Type  : void
//
static __global__ __launch_bounds__(1024, 1) void diff_unwrap_phase_kernel1(
    const emxArray_creal32_T yq, const int nx, emxArray_real32_T b_y)
{
...
// Return Type  : void
//
static __global__ __launch_bounds__(1024, 1) void diff_unwrap_phase_kernel2(
    const emxArray_real32_T b_y, const int vstride, const int i1,
                                          ...
                                          emxArray_real32_T *gpu)
{
  ...
  if (gpu->data == 0) {
    newData = 0ULL;
    cudaMalloc(&newData,
               static_cast<unsigned int>(cpu->allocatedSize) * sizeof(float));
    ...
    if (gpu->allocatedSize < actualSizeCpu) {
      ...
      cudaMemcpy(newData, gpu->data,
                 static_cast<unsigned int>(actualSizeGpu) * sizeof(float),
                                           ...
                                           const emxArray_creal32_T *cpu)
{
  ...
  if (gpu->allocatedSize < actualSize) {
    ...
    gpu->allocatedSize = i;
    gpu->canFreeData = true;
    cudaMalloc(&gpu->data, static_cast<unsigned int>(gpu->allocatedSize) *
                               sizeof(creal32_T));
  }
  cudaMemcpy(gpu->data, cpu->data,
             static_cast<unsigned int>(actualSize) * sizeof(creal32_T),
                                          ...
                                          const emxArray_real32_T *cpu)
{
  ...
  if (gpu->allocatedSize < actualSize) {
    ...
    gpu->canFreeData = true;
    cudaMalloc(&gpu->data,
               static_cast<unsigned int>(gpu->allocatedSize) * sizeof(float));
  }
  cudaMemcpy(gpu->data, cpu->data,
             static_cast<unsigned int>(actualSize) * sizeof(float),
             cudaMemcpyHostToDevice);
}
...
void diff_unwrap_phase(const emxArray_creal32_T *cpu_yq, emxArray_real32_T *b_z)
{
  ...
  validLaunchParams = mwGetLaunchParameters1D(static_cast<double>(nx), &grid,
                                              &block, 1024U, 65535U);
  ...
  for (nx = 0; nx < vstride; nx++) {
    ...
    boolean_T exitg1;
    i1++;
    validLaunchParams = mwGetLaunchParameters1D(static_cast<double>(i2 + 1LL),
    ...
    if (validLaunchParams) {
      ...
      diff_unwrap_phase_kernel2<<<grid, block>>>(gpu_y, vstride, i1, i2,
                                                 gpu_vwork);
      vwork_outdatedOnGpu = false;
    ...
    if (static_cast<int>(k) < cpu_vwork->size[0]) {
      ...
      do {
        exitg2 = 0;
        k++;
        while ((k <= static_cast<unsigned int>(m)) &&
               (std::isinf(cpu_vwork->data[static_cast<int>(k) - 1]) ||
                std::isnan(cpu_vwork->data[static_cast<int>(k) - 1]))) {
          k++;
        }
        if (k > static_cast<unsigned int>(m)) {
          ...
          pkm1 = cpu_vwork->data[static_cast<int>(k) - 1];
          cpu_vwork->data[static_cast<int>(k) - 1] -=
              6.28318548F * cumsum_dp_corr;
